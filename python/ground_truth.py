...
def bayes_rate_monte_carlo(data_type, data_params):
    """Computes the R2 bayes rate from Monte Carlo simulation.

    Parameters
    ----------
    data_type: str
        The type of data desired. Either `'MCAR'`, `'MAR'` or
        `'selfmasking'`. Within the selfmasking type, the Bayes predictor is
        only available for gaussian selfmasking.
    data_params: tuple
        The data parameters generated by either `gen_params`
        or `gen_params_selfmasking`.
    """

    if data_type in ['MCAR', 'MAR_logistic']:
        gen = gen_data
        bayes_predictor = BayesPredictor_MCAR_MAR(data_params)
    elif data_type == 'selfmasking' and data_params[1] == 'gaussian':
        gen = gen_data_selfmasking
        bayes_predictor = BayesPredictor_gaussian_selfmasking(data_params)
    else:
        raise ValueError("data type should be either 'mixture' or" +
                         "'gaussian_selfmasking'. There is no Bayes" +
                         "predictor available for other types of data.")

    X, y = next(gen([100000], data_params, random_state=0))
    risk = np.mean((bayes_predictor.predict(X) - y) ** 2)
